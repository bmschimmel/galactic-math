...
### IDT-309 — Clarify the Alien Invasion mode selector, keep the rules a surprise (PR #162)

Traced the per-mode rules in `pages/alien-invasion.html`: the alien count is the only thing Recon / Invasion / Chaos change, and it also decides who shoots back — nobody in Recon, half rounded up otherwise (5 of 10, 13 of 25), each shooter firing every 3–6.5 s when the ship is within 900 px. Lives, fuel, rings, comets and asteroids are the same in every mode. The one real gap: the mode-selector cards on the setup deck never said the big number (5/10/25) was a count of alien ships, so it now carries an `ALIENS` unit label, and the options hint spells out "The big number is how many alien ships attack." The how-to-play text's missile count was also wrong (said "a missile," code gives 2) and is now fixed. The per-mode sub-lines ("Alien ships unarmed" / "Aliens fire lasers!" / "Good luck, pilot") are unchanged — the exact shooter counts stay a surprise rather than being spelled out on the launch screen. The shooter-count logic was pulled into one `N_SHOOTERS` constant internally so the rule lives in one place. No gameplay change. `docs/game-modes.md` replaces its stale gate-clearing description with the real objective, a per-mode table and the laser rules, for developers.
