...
- **`.deck-body` reserves a minimum height** (340px, 370px on phones, tuned to each context's tallest step with a small cushion — IDT-297, IDT-300) so the nav row sits in the same place on every step. It never clips or scrolls: the card slide-in moves only 14px, inside the card's padding, and the tap bounce on an edge tile spills into that padding.
